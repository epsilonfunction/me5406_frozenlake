...
experiment_dir = experiment_dir.joinpath('SARSA/')
experiment_dir.mkdir(exist_ok=True)
...
DEFAULT = True # Use Default settings; False for custom map and custom size
MAP_LOAD_BOOL = False
...
for episode in range(total_episodes):
    ...
    # for step in range(max_steps):
        ...
        if exp_exp_tradeoff > epsilon:
            ...
            try:
                action_list = possible_actions(new_state) 
                row = list(qtable[new_state])
                goodq1 = []
                for i in action_list:
                    goodq1.append(row[i])

                max_value = np.max(goodq1)
                max_indices = np.where(row == max_value)[0]
                if len(max_indices) == 1:
                    new_action = max_indices[0]
                else:
                    # if there are multiple maximum indices, choose one randomly
                    new_action = np.random.choice(max_indices)
            except:
                new_action = np.argmax(qtable[new_state])
        else:
            action_list = possible_actions(new_state)
        ...
        # # input()
        # new_action = input("Action Here: ")
        # new_action = int(action)

        # To avoid being stuck on a particular pattern i guess?
        # if state == new_state and action == new_action:
        #     continue
        ## Might wanna add increasing incentive to explore by upping epsilon? idk
        ...
        td_S = learning_rate * (reward + gamma * qtable[new_state,new_action] - qtable[state, action])


        qtable[state, action] += td_S
